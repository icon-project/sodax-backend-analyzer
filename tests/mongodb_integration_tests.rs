use sodax_backend_analizer::{
    db::{
        find_all_a_token_addresses, find_all_reserve_addresses, find_all_reserves,
        find_all_variable_debt_token_addresses, get_reserve_data_for_a_token,
        get_reserve_data_for_reserve_token, get_reserve_data_for_variable_debt_token,
        get_orderbook, get_user_position,
    },
    evm::{get_last_block, get_balance_of},
};

// For async tests
use tokio;

// EVM tests
#[tokio::test]
async fn test_get_last_block() {
    let result = get_last_block().await;
    // dbg!(&result);
    assert!(
        result.is_ok(),
        "Should successfully retrieve the last block"
    );

    let block = result.unwrap();
    // Add assertions based on expected block data
    assert!(block > 0, "Block number should be greater than 0");
}

// Database tests
#[tokio::test]
async fn test_find_all_reserves() {
    let result = find_all_reserves().await;
    assert!(result.is_ok(), "Should successfully retrieve reserves");

    let reserves = result.unwrap();
    // Add assertions based on expected data
    assert!(!reserves.is_empty(), "Should have some reserves");
}

#[tokio::test]
async fn test_find_all_reserve_addresses() {
    let addresses = find_all_reserve_addresses().await;
    // Should return a Vec<String> even if empty
    assert!(addresses.is_empty() || !addresses.is_empty());
}

#[tokio::test]
async fn test_find_all_a_token_addresses() {
    let addresses = find_all_a_token_addresses().await;
    // Should return a Vec<String> even if empty
    // dbg!(&addresses);
    assert!(addresses.is_empty() || !addresses.is_empty());
}

#[tokio::test]
async fn test_find_all_variable_debt_token_addresses() {
    let addresses = find_all_variable_debt_token_addresses().await;
    // Should return a Vec<String> even if empty
    assert!(addresses.is_empty() || !addresses.is_empty());
}

#[tokio::test]
async fn test_get_reserve_data_for_reserve_token() {
    // Test with a known token address (you'd need to insert test data first)
    let test_token = "0xe801ca34e19abcbfea12025378d19c4fbe250131";
    let result = get_reserve_data_for_reserve_token(test_token).await;

    // This might return None if the token doesn't exist in test data
    match result {
        Ok(Some(_value)) => {
            // Token found
            // dbg!(_value);
        }
        Ok(None) => {
            // Token not found - this is valid for test data
        }
        Err(e) => {
            panic!("Database error: {}", e);
        }
    }
}

#[tokio::test]
async fn test_get_reserve_data_for_a_token() {
    // Test with a known token address (you'd need to insert test data first)
    let test_token = "0x5c50cf875aebad8d5ba548f229960c90b1c1f8c3";
    let result = get_reserve_data_for_a_token(test_token).await;

    // This might return None if the token doesn't exist in test data
    match result {
        Ok(Some(_value)) => {
            // Token found
            // dbg!(_value);
        }
        Ok(None) => {
            // Token not found - this is valid for test data
        }
        Err(e) => {
            panic!("Database error: {}", e);
        }
    }
}

#[tokio::test]
async fn test_get_reserve_data_for_variable_debt_token() {
    // Test with a known token address (you'd need to insert test data first)
    let test_token = "0x19c023ff9c8105bf58e022c17a636b8e55ed8fe4";
    let result = get_reserve_data_for_variable_debt_token(test_token).await;

    // This might return None if the token doesn't exist in test data
    match result {
        Ok(Some(_value)) => {
            // Token found
            // dbg!(_value);
        }
        Ok(None) => {
            // Token not found - this is valid for test data
        }
        Err(e) => {
            panic!("Database error: {}", e);
        }
    }
}

#[tokio::test]
async fn test_get_orderbook() {
    let result = get_orderbook().await;
    assert!(result.is_ok(), "Should successfully retrieve orderbook data");

    let _orderbook = result.unwrap();
    // Orderbook can be empty, which is valid
    // dbg!(&_orderbook);
}

#[tokio::test]
async fn test_get_user_position() {
    // Test with a known user address (you'd need to insert test data first)
    let test_user = "0xf754037f99af3d90a4db611d74966729b81a8a96";
    let result = get_user_position(test_user).await;

    // This might return None if the user doesn't exist in test data
    match result {
        Ok(Some(_value)) => {
            // User position found
            // dbg!(_value);
        }
        Ok(None) => {
            // User not found - this is valid for test data
        }
        Err(e) => {
            panic!("Database error: {}", e);
        }
    }
}

#[tokio::test]
async fn test_get_balance_of() {
    // Test with valid Ethereum addresses
    let token_address = "0x5c50cf875aebad8d5ba548f229960c90b1c1f8c3"; // Example token
    let user_address = "0xf754037f99af3d90a4db611d74966729b81a8a96"; // Example user
    
    let result = get_balance_of(token_address, user_address).await;
    
    // The result might be Ok or Err depending on the network and contract state
    // We just test that the function doesn't panic and handles the request
    match result {
        Ok(_balance) => {
            // Balance retrieved successfully
            // dbg!(_balance);
        }
        Err(_e) => {
            // Error is acceptable if the contract doesn't exist or network issues
            // dbg!(_e);
        }
    }
}
